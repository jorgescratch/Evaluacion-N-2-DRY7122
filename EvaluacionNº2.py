...
GH_API_KEY = "9c75d513-52f7-45eb-a19d-ed34addc3a8e"
...
def obtener_datos_viaje(origen, destino):
    ...
    url = f"https://graphhopper.com/api/1/route?point={origen_lat},{origen_lon}&point={destino_lat},{destino_lon}&vehicle=car&locale=es&key={GH_API_KEY}&instructions=true"
    response = requests.get(url)
    data = response.json()
...
def imprimir_narrativa(narrativa):
    ...
    for paso in narrativa:
        distancia = paso['distance'] / 1000.0  # Convertir de metros a kilómetros
        tiempo = paso['time'] / 1000.0  # Convertir de milisegundos a segundos
        print(f"{paso['text']} - {distancia:.2f} km, {tiempo:.2f} segundos")
